...
//=============================================================================
{
	...
	if(hSource == NULL)
		...
		TRACE(_T("@0CPicture::Load FindResource() Failed.\r\n"));
		return(FALSE);
		}
	...
	if(hGlobal == NULL)
		{
		HWND hWnd = AfxGetApp()->GetMainWnd()->m_hWnd;
		TRACE(_T("@0CPicture::Load LoadResource() Failed.\r\n"));
		return(FALSE);
		}
	...
	if(lpVoid == NULL)
		{
		HWND hWnd = AfxGetApp()->GetMainWnd()->m_hWnd;
		TRACE(_T("@0CPicture::Load LockResource() Failed.\r\n"));
		return(FALSE);
		}
...
// ~~~~~~
//-----------------------------------------------------------------------------
BOOL CPicture::Load(LPCTSTR sFilePathName)
//=============================================================================
{
	BOOL bResult = FALSE;
	CFile PictureFile;
	...
	if(PictureFile.Open(sFilePathName, CFile::modeRead | CFile::typeBinary, &e))
		...
		TRACE(_T("@1CPicture::Load. Open Failed: %s\n"), szCause);
		//HWND hWnd = AfxGetApp()->GetMainWnd()->m_hWnd;
...
//=============================================================================
{
	...
	if(hGlobal == NULL)
		{
		HWND hWnd = AfxGetApp()->GetMainWnd()->m_hWnd;
		TRACE(_T("CPicture::LoadPictureData. Can not allocate enough memory\t"));
		return(FALSE);
		}
	...
	if(CreateStreamOnHGlobal(hGlobal, TRUE, &pStream) == S_OK)
		...
		if((hr = OleLoadPicture(pStream, nSize, FALSE, IID_IPicture, (LPVOID *)&m_IPicture)) == E_NOINTERFACE)
			{
			HWND hWnd = AfxGetApp()->GetMainWnd()->m_hWnd;
			TRACE(_T("CPicture::LoadPictureData. IPicture interface is not supported\n"));
			return(FALSE);
			}
		else // S_OK
			{
...
//=============================================================================
{
	...
	HWND hWnd = AfxGetApp()->GetMainWnd()->m_hWnd;
	TRACE(_T("CPicture::Show. Can not allocate enough memory\n"));
    return(FALSE);
}
...
//=============================================================================
{
	...
	HWND hWnd = AfxGetApp()->GetMainWnd()->m_hWnd;
	TRACE(_T("CPicture::Show2. Can not allocate enough memory\n"));
    return(FALSE);
}
...
// ~~~~~~
//-----------------------------------------------------------------------------
BOOL CPicture::SaveAsBitmap(LPCTSTR sFilePathName)
//=============================================================================
{
	BOOL bResult = FALSE;
	ILockBytes *Buffer = 0;
	...
	if(BufferBytes == NULL)
		...
		Buffer->Release();
		HWND hWnd = AfxGetApp()->GetMainWnd()->m_hWnd;
		TRACE(_T("CPicture::SaveAsBitmap. Can not allocate enough memory\n"));
	...
	if(BitmapFile.Open(sFilePathName, CFile::typeBinary | CFile::modeCreate | CFile::modeWrite, &e))
		...
		e.GetErrorMessage(szCause, 255, NULL);
		HWND hWnd = AfxGetApp()->GetMainWnd()->m_hWnd;
		TRACE(_T("CPicture::SaveAsBitmap. Write File Failed\n"));
		bResult = FALSE;
		}
...
//=============================================================================
{
   ...
   if(BMP.LoadBitmap(BMPResource))
		...
		{
		TRACE("@0CPicture::ShowBitmapResource Can Not Find The Bitmap Resource\n");
		return(FALSE);
		}
