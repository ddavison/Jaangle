...
#ifdef _UNITTESTING
BOOL TestCFolderDialog()
{
	CFolderDialog test1(_T("Test 1: Null Root"), NULL, NULL, BIF_RETURNONLYFSDIRS | BIF_USENEWUI);
	if(test1.DoModal() == IDOK)
		MessageBox(NULL, test1.GetFolderPath(), _T("Result"), MB_OK);
	
	CFolderDialog test2(_T("Test 2: Windows Root"), _T("c:\\windows"), NULL, BIF_RETURNONLYFSDIRS | BIF_USENEWUI);
	if (test2.DoModal() == IDOK)
		MessageBox(NULL, test2.GetFolderPath(), _T("Result"), MB_OK);

	CFolderDialog test3(_T("My Preference"), _T("c:\\windows"), NULL, BIF_RETURNONLYFSDIRS | BIF_UAHINT | BIF_NEWDIALOGSTYLE | BIF_NONEWFOLDERBUTTON);
	if (test3.DoModal() == IDOK)
		MessageBox(NULL, test3.GetFolderPath(), _T("Result"), MB_OK);
	...
	const Flags flags[] ={
		{BIF_BROWSEFORCOMPUTER, _T("BIF_BROWSEFORCOMPUTER")},
		{BIF_BROWSEFORPRINTER, _T("BIF_BROWSEFORPRINTER")},
		{BIF_BROWSEINCLUDEFILES, _T("BIF_BROWSEINCLUDEFILES")},
		{BIF_DONTGOBELOWDOMAIN, _T("BIF_DONTGOBELOWDOMAIN")},
		{BIF_EDITBOX, _T("BIF_EDITBOX")},
		{BIF_NEWDIALOGSTYLE, _T("BIF_NEWDIALOGSTYLE")},
		{BIF_NONEWFOLDERBUTTON, _T("BIF_NONEWFOLDERBUTTON")},
		{BIF_RETURNFSANCESTORS, _T("BIF_RETURNFSANCESTORS")},
		{BIF_RETURNONLYFSDIRS, _T("BIF_RETURNONLYFSDIRS")},
		{BIF_STATUSTEXT | BIF_NEWDIALOGSTYLE, _T("BIF_STATUSTEXT | BIF_NEWDIALOGSTYLE")},
		{BIF_UAHINT | BIF_NEWDIALOGSTYLE, _T("BIF_UAHINT | BIF_NEWDIALOGSTYLE")},
		{BIF_USENEWUI, _T("BIF_USENEWUI")},
		{0, _T("")}
	};

	int flCount = 0;
	while (flags[flCount].flag)
	{
		CFolderDialog test(flags[flCount].flagDesc, _T("c:\\windows"), NULL, flags[flCount].flag);
		if (test.DoModal() == IDOK)
		{
			if (MessageBox(NULL, test.GetFolderPath(), _T("Continue Testing?"), MB_YESNO) == IDNO)
				break;
		}
		flCount++;
	}
	return MessageBox(NULL, _T("Were the tests succesfull?"), _T("Result"), MB_YESNO) == IDYES;
}
#endif
...
{
	ASSERT( AfxIsValidString( pszPath, MAX_PATH ) );
	return (BOOL)::_tcsncpy(m_szSelPath, pszPath, MAX_PATH);
}
...
VOID CFolderDialog::OnSelChanged( IN LPITEMIDLIST pItemIDList )
{
	...
	if(m_AllowedDriveType > 0)
	{
		...
		{
			UINT drvType;
			if (szSelFol[0] == '\\' && szSelFol[1] == '\\')
				drvType= DRIVE_REMOTE;
			else
			{
				szSelFol[3] = 0;
				drvType = GetDriveType(szSelFol);
			}
			bAllow = (m_AllowedDriveType == drvType);
		}
		SendMessage(BFFM_ENABLEOK, 0, bAllow);
	}
}
