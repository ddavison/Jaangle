...
m_iconX(16),
m_iconY(15),
m_pExternalFont(NULL),
m_pInternalFont(NULL)
{
...
{
	DestroyMenu();
	delete m_pInternalFont;
}

CFont* BCMenu::GetFont()
{
	if (m_pExternalFont != NULL)
		return m_pExternalFont;
	if (m_pInternalFont == NULL)
	{
		m_pInternalFont = new CFont;
		LOGFONT lf;
		...
		lf =  nm.lfMenuFont;
		m_pInternalFont->CreateFontIndirect (&lf);
	}
	return m_pInternalFont;
}
...
BOOL BCMenu::DestroyMenu()
{
	...
	for(int m = 0; m <= numItems; m++)delete(m_MenuList[m]);
...
void BCMenu::MeasureItem( LPMEASUREITEMSTRUCT lpMIS )
{
	...
	if(state & MF_SEPARATOR){
		...
		if (IsNewShell())
			pFont = pDC->SelectObject (GetFont());// Select menu font in...
		...
		lpMIS->itemWidth += 20;//By ALEX. Looks better this way when there is a submenu and the arrow appears
...
BOOL BCMenu::AppendMenuW(UINT nFlags,UINT nIDNewItem,wchar_t *lpszNewItem, HICON hIcon)
{
	...
	if(hIcon){
		...
		if(IsLunaMenuStyle()&&xp_draw_3D_bitmaps)
		{
			...
			CDC *pDC = pWnd->GetDC();              // Get device context
			CBitmap bmp,bmp2,bmp3;
			GetBitmapFromImageList(pDC,mdata->bitmap,0,bmp);
			GetFadedBitmap(bmp);
			mdata->bitmap->Add(&bmp,GetSysColor(COLOR_3DFACE));
			GetBitmapFromImageList(pDC,mdata->bitmap,0,bmp2);
			GetShadowBitmap(bmp2);
			mdata->bitmap->Add(&bmp2,GetSysColor(COLOR_3DFACE));
			GetBitmapFromImageList(pDC,mdata->bitmap,0,bmp3);
			GetDisabledBitmap(bmp3);
			mdata->bitmap->Add(&bmp3,GetSysColor(COLOR_3DFACE));
			pWnd->ReleaseDC(pDC);  // Release the DC
		}
...
BOOL BCMenu::AppendMenuA(UINT nFlags,UINT nIDNewItem,const char *lpszNewItem,HICON hIcon)
{
	USES_CONVERSION;
	return AppendMenuW(nFlags,nIDNewItem,A2W(lpszNewItem),hIcon);
}
