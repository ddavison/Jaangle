...
void CButtonST::FreeResources(BOOL bCheckForNULL)
{
	if (bCheckForNULL)
	{
		...
		if (m_csIcons[0].hIcon && m_csIcons[0].bAllowDestroy)	::DestroyIcon(m_csIcons[0].hIcon);
		if (m_csIcons[1].hIcon && m_csIcons[1].bAllowDestroy)	::DestroyIcon(m_csIcons[1].hIcon);
...
DWORD CButtonST::SetIcon(int nIconIn, int nCxDesiredIn, int nCyDesiredIn, int nIconOut, int nCxDesiredOut, int nCyDesiredOut)
{
	...
	return SetIcon(hIconIn, hIconOut, TRUE);
} // End of SetIcon
...
//			Failed loading the specified resource.
//
DWORD CButtonST::SetIcon(HICON hIconIn, HICON hIconOut, BOOL bAllowDestroy)
{
	BOOL		bRetValue;
	ICONINFO	ii;
	...
	if (hIconIn)
	{
		...
		m_csIcons[0].bAllowDestroy = bAllowDestroy;
		// Get icon dimension
		...
		if (hIconOut)
		{
			...
			m_csIcons[1].bAllowDestroy = bAllowDestroy;
			// Get icon dimension
			::ZeroMemory(&ii, sizeof(ICONINFO));
