...
//=============================================================================
{
	...
	if (!m_strOkTitle.IsEmpty())
		pWndParent->SetDlgItemText(IDOK, m_strOkTitle);
	if (!m_strCancelTitle.IsEmpty())
		pWndParent->SetDlgItemText(IDCANCEL, m_strCancelTitle);
	if (!m_strFolderNameTitle.IsEmpty())
	{
		CWnd* wnd = GetDlgItem(IDC_MRU_CAPTION);
		if (wnd != NULL)
			wnd->SetWindowText(m_strFolderNameTitle);
	}
	if (!m_strLookInTitle.IsEmpty())
	{
		//=== Code to find the number 1091
		//CWnd* pSt = pWndParent->FindWindowEx(pWndParent->m_hWnd, NULL, _T("STATIC"), NULL);
		//INT nID = pSt->GetDlgCtrlID();
		pWndParent->SetDlgItemText(1091, m_strLookInTitle);
...
//=============================================================================
{
	...
	if (GetWindowsVersion(verInfo))
		return verInfo.osCode >= VST_Win2000;
	return FALSE;
}
