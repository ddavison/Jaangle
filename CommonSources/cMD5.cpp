...
char* cMD5::CalcMD5FromFile(LPCTSTR path)
{
	...
	HANDLE h_File = CreateFile(path, GENERIC_READ, FILE_SHARE_READ, 0, 
...
char* cMD5::CalcMD5FromWString(const wchar_t *buffer)
{
	MD5Init();
	MD5Update((unsigned char*)buffer, wcslen(buffer) * sizeof(wchar_t));
	return MD5FinalToString();
}

char* cMD5::CalcMD5FromBuffer(const BYTE *s8_Input, UINT bfLen)
{
	MD5Init();
	MD5Update((unsigned char*)s8_Input, bfLen);
	return MD5FinalToString();
}
