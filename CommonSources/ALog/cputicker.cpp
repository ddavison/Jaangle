...
// 23/02/04 PJ Naughter
//              1. Now uses VC 6 SP5's and later support for built in assembler support for
//              cpuid and rdtsc. Thanks to Matthew Hurd for reporting this. This does mean
//              that the code will only compile on VC 6 SP5 or later now.
//
// LEGALITIES:
// Copyright © 1996-2004 by J.M.McGuiness and PJ Naughter, all rights reserved.
//
...
#include "stdafx.h"
#ifndef _INC_MATH
#pragma message("To avoid this message please put math.h into your PCH (normally stdafx.h)")
#include <math.h>
#endif
#include "cputicker.h"
...
CCPUTicker::CCPUTicker()
{
  ...
  if (!m_bStaticsCalculated)
  {
	...
	GetCPUFrequency(m_freq, m_deviation);
  }
}
...
#pragma optimize("",off)
BOOL CCPUTicker::GetCPUFrequency(double& frequency, double& target_ave_dev, unsigned long interval, unsigned int max_loops)
{
	...
	TRACE(_T("@3 CCPUTicker::GetCPUFrequency freq: %d MHz\r\n"),INT(ave_freq/1000000.0));
	frequency = ave_freq;
