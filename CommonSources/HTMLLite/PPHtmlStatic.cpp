...
BEGIN_MESSAGE_MAP(CPPHtmlStatic, CStatic)
	...
	ON_MESSAGE(UDM_STATIC_REPAINT, OnRepaintWindow)
END_MESSAGE_MAP()
...
//	SetWindowLong(GetSafeHwnd(), GWL_EXSTYLE, dwExStyles);
	...
	m_drawer.SetCallbackRepaint(GetSafeHwnd(), UDM_STATIC_REPAINT);

	CFont* pFont = GetFont();
	HFONT hFont = NULL;
	//ENG: Gets a default system font
	//RUS: Οξλσχΰεμ ρθρςεμνϋι ψπθτς οξ-σμξλχΰνθώ
	if (NULL == hFont)
	{
...
{
	_tcsncpy((LPTSTR)lParam, m_sText, wParam);
	return 1;
...
*/
	...
	//RUS: Οξλξζενθε ξαλΰρςθ βϋβξδΰ οξ ξςνξψενθώ κ κλθενςρκθμ κξξπδθνΰςΰμ
...
afx_msg LRESULT CPPHtmlStatic::OnRepaintWindow(WPARAM wParam, LPARAM lParam)
{
	...
	RepaintWindow();
    return TRUE;
} //End of the UDM_STATIC_REPAINT handler
