...
void CPPToolTip::OnRedrawTooltip(HDC hDC, BYTE nTransparency /* = 0 */)
{
	...
	//RUS: Οξλσχΰεμ πΰημεπ ςσλςθοΰ αεη ςενθ
...
//	GetWindowRgn(hrgn);
	...
	if (NULL == m_hUnderTooltipBk)
	{
		...
		//RUS: Ρξυπΰνεμ τξν οξδ ςσλςθοξμ β αθςμΰο
	...
	else
	{
		...
		//RUS: Βξρρςΰνΰβλθβΰεμ τξν θη αθςμΰοΰ
		::SelectObject(hBkDC, m_hUnderTooltipBk);
	} //if

	//ENG: Copy a original background bitmap to the temporary DC
	//RUS: Κξοθπσεμ ξπθγθνΰλόνϋι αθςμΰο βξ βπεμεννϋι κξνςεκρς οΰμςθ
	...
	//RUS: Νΰκλΰδϋβΰεμ ςσλςθο νΰ κλθενςρκσώ χΰρςό ρ ΰλότΰ-νΰλξζενθεμ
	...
	//RUS: Ξρβξαξζδΰεμ κξνςεκρς σρςπξιρςβΰ ερλθ ύςξ νεξαυξδθμξ
	if (bAutoReleaseDC)
...
BOOL CPPToolTip::RelayEvent(MSG* pMsg)
{
	...
	//RUS: Ηΰοπες ξαπΰαξςκθ ρξξαωενθι ερλθ ξαϊεκς νε ρξηδΰν
	if (NULL == GetSafeHwnd())  
		return FALSE;
	...
	case WM_MOUSEMOVE:
		...
		if ((m_ptOriginal.x != pMsg->pt.x) || (m_ptOriginal.y != pMsg->pt.y))
		{
			...
			else if (IsCursorOverTooltip() && !(m_tiDisplayed.nBehaviour & PPTOOLTIP_TRACKING_MOUSE)) 
			{
				...
				//RUS: Κσπρξπ νΰδ ςσλςθοξμ οπθ βϋκλώχεννξμ πεζθμε "ςπΰκθνγΰ"
				if (!(m_tiDisplayed.nBehaviour & PPTOOLTIP_NOCLOSE_OVER))
				{
					//ENG: A tooltip don't hides when mouse over him
					//RUS: Ερλθ νε σρςΰνξβλεν ρςθλό νε ηΰκπϋβΰνθ ςσλςθοΰ ερλθ κσπρξπ νΰδ νθμ
					HideTooltip();
				}
				else
				{
					//ENG: Resetup autopop timer
					//RUS: Οεπεσρςΰνΰβλθβΰεμ ςΰιμεπ ΰβςξηΰκπϋςθ ςσλςθοΰ
					SetAutoPopTimer();
				} //if
			}
			else
			{
				...
				//RUS: Θωεμ ύλεμενς νΰ οΰνελθ θνρςπσμενςξβ
				hWnd = FindToolBarItem(pMsg->pt, ti);
				if (NULL == hWnd)
				{
					//ENG: Searching a hot area of the tooltip
					//RUS: Θωεμ οπεδξοπεδελεννσώ γξπχσώ ηξνσ ςσλςθοΰ
					hWnd = FindTool(&pt, ti);
				} //if
				if (NULL == hWnd)
				{
					//ENG: An item with a tooltip wasn't found
					//RUS: Νθ ξδθν ύλεμενς, ξςξαπΰζΰώωθι ςσλςθο, νε νΰιδεν
					m_hwndDisplayedTool = NULL;
				...
				else 
				{
					if ((hWnd != m_hwndDisplayedTool) || (ti.rectBounds != m_tiDisplayed.rectBounds/* m_rcDisplayedTool*/))
					{
						...
						//RUS: Ερλθ νξβξε ξκνξ θλθ νξβϋι ύλεμενς ξκνΰ, ςξ σρςΰνξβθςό νξβϋι ςσλςθο
						SetNewTooltip(hWnd, ti);
					}
					else
					{
						//ENG: Nothing was changed
						//RUS: Ερλθ νθ ξκνξ, νθ ύλεμενς ξκνΰ νε θημενλθρό
						if (m_tiDisplayed.nBehaviour & PPTOOLTIP_TRACKING_MOUSE)
						{
							//ENG: If sets tracking mode
							//RUS: Ερλθ σρςΰνξβλεν πεζθμ "ςπΰκθνγΰ"
							SetAutoPopTimer();
						...
						else if (!(m_tiDisplayed.nBehaviour & PPTOOLTIP_CLOSE_LEAVEWND))
						{
							...
							//RUS: Ερλθ δξλζεν οπςΰςόρ οπθλώαξμ δβθζενθθ μϋψθ
							if ((hWnd == m_hwndDisplayedTool) && 
								!(m_tiDisplayed.nBehaviour & PPTOOLTIP_MULTIPLE_SHOW))
							{
								//ENG: "Multiple show" mode was disabled
								//RUS: Ερλθ νε σρςΰνξβλεν πεζθμ μνξζερςβεννξγξ οξκΰηΰ ςσλςθοΰ
								HideTooltip();
							}
							else
							{
								//ENG: "Multiple show" mode was enabled
								//RUS: Ερλθ σρςΰνξβλεν πεζθμ μνξζερςβεννξγξ οξκΰηΰ ςσλςθοΰ
								SetNewTooltip(hWnd, ti);
							} //if
						}
						else
						{
							...
							//RUS: σλςθο νε δξλζεν οπςΰςόρ οξκΰ νΰυξδθςρ νΰδ ξκνξμ
							SetAutoPopTimer();
						} //if
					} //if
				} //if
...
void CPPToolTip::OnTimer(UINT nIDEvent) 
{
	...
	case TIMER_HIDE:
		...
		if (PPTOOLTIP_STATE_HIDING != m_nTooltipState)
		{
			...
			if ((SHOWEFFECT_FADEINOUT == m_dwHideEffect) && m_dwTimeFadeOut)
			{
				...
				//RUS: Ερλθ ύττεκς ολΰβνξγξ ρξκπϋςθ πΰηπεψεν θ σκΰηΰνξ βπεμ ψΰγΰ ρξκπϋςθ
				SetTimer(TIMER_HIDING, m_dwTimeFadeOut, NULL);
			}
			else
			{
				//ENG: Sets a maximum transparency and to stops a hiding of a tooltip
				//RUS: Σρςΰνΰβλθβΰεμ μΰκρθμΰλόνσώ οπξηπΰχνξρςό θ ξρςΰνΰβλθβΰεμ ρξκπϋςθε
	...
	case TIMER_HIDING:
		...
		//RUS: Ερλθ ψΰγθ ύττεκςΰ ολΰβνξγξ ρξκπϋςθ θρχεποΰνϋ, ςξ σρςΰνξβθςό μΰκρθμΰλόνσώ οπξηπΰχνξρςό
		...
		if (PERCENT_MAX_TRANSPARENCY == m_dwCurTransparency)
		{
			...
			//RUS: Σαθβΰεμ ςΰιμεπ ολΰβνξγξ ρξκπϋςθ θ οπχεμ ξκνξ ςσλςθοΰ
			...
			if (m_bNextToolExist)
			{
				...
				//RUS: Ερλθ ρσωερςβσες οξδγξςξβλεννϋι ςσλςθο, ςξ νΰχΰςό εγξ ξςξαπΰζενθε
				m_bNextToolExist = FALSE;
//				m_nTooltipType = m_nNextTooltipType;
//				m_nNextTooltipType = PPTOOLTIP_NORMAL;
			...
			else
			{
				...
				//RUS: Ερλθ οξδγξςξβλεννϋι ςσλςθο νε ρσωερςβσες
				m_nTooltipType = PPTOOLTIP_NORMAL;
				m_nNextTooltipType = PPTOOLTIP_NORMAL;
			} //if
		}
		...
		{
			//ENG: Redraw tooltip with new transparency factor
			//RUS: Οεπεπθρξβΰςό ςεκσωθι ςσλςθο ρ νξβϋμ κξύττθφθενςξμ οπξηπΰχνξρςθ
			OnRedrawTooltip(NULL, m_dwCurTransparency);
		} //if
		break;	
...
void CPPToolTip::HideTooltip()
{
	...
	case PPTOOLTIP_STATE_SHOWING:
		...
		//RUS: Ερλθ ςσλςθο ςξλόκξ ξςαπΰζΰεςρ, ςξ οπεκπΰωΰεμ εγξ ξςξαπΰζΰςό
		KillTimer(TIMER_SHOWING);
...
void CPPToolTip::PrepareDisplayTooltip(LPPOINT lpPoint)
{
	...
	//RUS: Ρξηδΰεμ αθςμΰο τξνΰ ςσλςθοΰ
	...
	//RUS: Ρξηδΰνθε τξνΰ ςελΰ ςσλςθοΰ
	...
	//RUS: Βϋχθρλενθε οξλξζενθ ςσλςθοΰ νΰ ύκπΰνε
	...
	//RUS: Ερλθ ολΰβνξε ξςξαπΰζενθε δξρςσονξ
	if ((SHOWEFFECT_FADEINOUT == m_dwShowEffect) && m_dwTimeFadeIn)
	...
	if (m_szOffsetShadow.cx || m_szOffsetShadow.cy)
	{
		...
		//RUS: Ερλθ ςενό ασδες ξςξαπΰζΰςόρ
		...
		//RUS: Σβελθχθβΰεμ πΰημεπϋ ςσλςθοΰ δλ πθρξβΰνθ ςενθ
...
DWORD CPPToolTip::GetTooltipDirection(DWORD dwDirection, const CPoint & ptPoint, CPoint & ptAnchor, CRect & rcBody, CRect & rcFull, CRect & rcTipArea)
{
	...
	//RUS: Οξλσχΰεμ οξλνϋι οπμξσγξλόνθκ ύκπΰνΰ Windows
	CRect rWindow;
	...
	//RUS: Θνθφθΰλθηΰφθ οξλνξγξ πΰημεπΰ ξγπΰνθχθβΰώωεγξ ςσλςθο οπμξσγξλόνθκΰ
	rcFull = rcBody;
	switch(dwDirection) 
	{
	case PPTOOLTIP_LEFTEDGE_TOP:
	...
	//RUS: Οπξβεπκΰ γξπθηξνςΰλόνϋυ πΰημεπξβ νΰ οξοΰδΰνθε β ύκπΰν
	CPoint pt(ptPoint);
	switch(dwDirection) 
	{
	case PPTOOLTIP_LEFTEDGE_TOP:
	...
	//RUS: Οπξβεπκΰ βεπςθκΰλόνϋυ πΰημεπξβ νΰ οξοΰδΰνθε β ύκπΰν
	switch(dwDirection) 
	{
	case PPTOOLTIP_LEFTEDGE_TOP:
	case PPTOOLTIP_RIGHTEDGE_TOP:
	...
	//RUS: Σρςΰνξβκΰ κξξπδθνΰςϋ κξνχθκΰ
	switch(dwDirection) 
	{
	case PPTOOLTIP_LEFTEDGE_TOP:
	case PPTOOLTIP_LEFTEDGE_VCENTER:
...
BOOL CPPToolTip::FindTool(HWND hWnd, const LPPOINT lpPoint, PPTOOLTIP_INFO & ti)
{
	...
	if ((hotarea.size() == 1) && hotarea[0].rectBounds.IsRectEmpty())
	{
		...
		//RUS: Ερλθ ξγπΰνθχθβΰώωθι οπμξσγξλόνθκ γξπχει ηξνϋ νε ξοπεδελεν
		ti = hotarea[0];
		return TRUE;
	} //if
	...
	if (hWnd != m_hParentWnd)
	{
		...
		//RUS: Ερλθ HWND νε ξςνξρθςρ κ πξδθςελόρκξμσ ξκνσ, ςξ οπεξαπΰησεμ κξξπδθνΰςϋ
	...
	if (pScroll->IsKindOf(RUNTIME_CLASS(CScrollView))) 
	{
		...
		//RUS: Ερλθ HWND οπθνΰδλεζθς CScrollView θλθ οπξθηβξδνξμσ ξς νεγξ κλΰρρσ, ςξ κξππεκςθπσεμ κξξπδθνΰςϋ
	...
	//RUS: Θωεμ γξπχσώ ηξνσ οξδ κσπρξπξμ
			...
			if (m_wndToolBars[i] == hWnd)
			{
				...
				for (int i = 0; i < count; i++)
				{
					...
					if (rect.PtInRect(point))
					{
						...
						//RUS: έλεμενς οΰνελθ θνρςπσμενςξβ ξανΰπσζεν
...
void CPPToolTip::AddTool(CWnd * pWnd, PPTOOLTIP_INFO & ti)
{
	...
	if (item == m_ToolMap.end())
	{
		...
		//RUS: σλςθο δλ σκΰηΰννξγξ HWND νε ξανΰπσζεν, οξύςξμσ ρξηδΰεμ εγξ
		arHotArea hotarea;
	...
	//RUS: Οξλσχΰεμ οΰπΰμεςπϋ ςσλςθοΰ
	arHotArea & hotarea = item->second;

	//ENG: A tooltip has more one rectangle areas. Check all theirs
	//RUS: σλςθο ρξδεπζθς αξλεε ξδνξι οπμξσγξλόνξι ξαλΰρςθ, οπξβεπθμ βρευ θυ
	arHotArea::iterator iter;
	for (iter = hotarea.begin(); iter != hotarea.end(); ++iter)
	{
		if (ti.rectBounds == (*iter).rectBounds)
		{
			...
			//RUS: Σκΰηΰννϋι οπμξσγξλόνθκ ξκνΰ σζε ρσωερςβσες, οξύςξμσ οπξρςξ ξανξβλεμ εγξ οΰπΰμεςπϋ
			*iter = ti;
			return;
		} //if
	} //for
	...
	//ENG: Adds a new tool 
	//RUS: Δξαΰβλεμ νξβϋι θνρςπσμενς
	hotarea.push_back(ti);
} //End of AddTool
...
void CPPToolTip::RemoveTool(CWnd * pWnd, LPCRECT lpRectBounds /* = NULL */)
{
	...
	if (NULL == lpRectBounds)
	{
		...
		//RUS: Σδΰλώςρ βρε θνρςπσμενςϋ δλ σκΰηΰννξγξ ξκνΰ
		m_ToolMap.erase(item);
	}
	else
	{
		//ENG: Search the tool to remove
		//RUS: Οξθρκ θνρςπσμενςΰ δλ σδΰλενθ
		arHotArea & hotarea = item->second;
		arHotArea::iterator iter;
		for (iter = hotarea.begin(); iter != hotarea.end(); ++iter)
		{
			if ((*iter).rectBounds == *lpRectBounds)
			{
				...
				//RUS: Θνρςπσμενς νΰιδεν
				if (hotarea.size() > 1)
				{
					//ENG: If the specified window has more one rectangle areas then removes the specified area only
					//RUS: Ερλθ σκΰηΰννξε ξκνξ ρξδεπζθς αξλεε ξδνξι ξαλΰρςθ, ςξ σδΰλεμ ςξλόκξ σκΰηΰννσώ ξαλΰρςό
					hotarea.erase(iter);
				}
				else
				{
					//ENG: If the specified window has one rectangle area only then removes the tool for specified window
					//RUS: Ερλθ σκΰηΰννξε ξκνξ θμεες ςξλόκξ ξδνσ οπμξσγξλόνσώ ξαλΰρςό, ςξ σδΰλθςό βερό θνρςπσμενς
					m_ToolMap.erase(item);
				} //if
				return;
			} //if
...
void CPPToolTip::RemoveAllTools()
{
	...
	//RUS: Σδΰλεμ βρε θνρςπσμενςϋ
	if (m_ToolMap.size())
		m_ToolMap.clear();

	//ENG: Removes all toolbars
	//RUS: Σδΰλεμ βρε οΰνελθ θνρςπσμενςξβ
	if (m_wndToolBars.size())
		m_wndToolBars.clear();
...
void CPPToolTip::AddToolBar(CToolBar * pBar)
{
	...
	//RUS: Οξλσχΰεμ HWND ξκνΰ οΰνελθ θνρςπσμενςξβ
	HWND hWnd = pBar->GetSafeHwnd();

	//ENG: Searching a clone of a toolbar
	//RUS: Οξθρκ δσαλθκΰςΰ σκΰηΰννξι οΰνελθ θνρςπσμενςξβ
	...
	//RUS: Ηΰοξμθνΰεμ HWND ξκνΰ οΰνελθ θνρςπσμενςξβ
	m_wndToolBars.push_back(hWnd);

	//ENG: Disables a standard tooltip for the specified toolbar
	//RUS: Ηΰοπεωΰεμ ρςΰνδΰπςνϋε οξδρκΰηκθ δλ σκΰηΰνξι οΰνελθ θνρςπσμενςξβ
