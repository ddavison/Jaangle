...
// -----------------------------------------------------------------------------
        ...
        const std::locale& loc=std::locale())
    {
		...
		if ( nSrc > 0 )
		{
			...
			SSCodeCvt::state_type st= { 0 };
			res						= conv.out(st,
										pSrcW, pSrcW + nDst, pNextSrcW,
										pDstA, pDstA + nSrc, pNextDstA);
...
inline void	ssasn(std::string& sDst, PCWSTR pW)
{
	...
	if ( nSrc > 0 )
	{
		...
		// In MBCS builds, pad the buffer to account for the possibitlity of
		// some 3 byte characters.  Not perfect but should get most cases.
...
public:
    ...
    MYTYPE& ToUpper(const std::locale& loc=std::locale())
	{
		...
		// for a character takes up a different number of bytes than the
		...
		// seems to be a bit faster when anything other then the "C" local is
		// used...
...
//		}
	...
	{
		MYTYPE sFormat;
		VERIFY(0 != sFormat.LoadString(nFormatId));
		va_list argList;
	...
	{
		return Mid(nFirst, this->size()-static_cast<MYSIZE>(nFirst));
	}
...
{
	CStdStringA sRet;
	int nObjSize = sA.size();
	int nLitSize = sslen(pA);
...
{
	CStdStringW sRet;
	int nObjSize = sW.size();
	int nLitSize = sslen(pW);
	sRet.reserve(nLitSize + nObjSize);
	sRet.assign(pW);
	sRet.append(sW);
	return sRet;
